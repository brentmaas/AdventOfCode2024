...
for frequency in antennae:
    for i in range(len(antennae[frequency]) - 1):
        for j in range(i + 1, len(antennae[frequency])):
            antinodes.add((antennae[frequency][i][0], antennae[frequency][i][1]))
            antinodes.add((antennae[frequency][j][0], antennae[frequency][j][1]))
            ...
            while 0 <= x1 < len(field[0]) and 0 <= y1 < len(field):
                antinodes.add((x1, y1))
                x1 += dx
                y1 += dy
            while 0 <= x2 < len(field[0]) and 0 <= y2 < len(field):
                antinodes.add((x2, y2))
                x2 -= dx
                y2 -= dy
print(len(antinodes))